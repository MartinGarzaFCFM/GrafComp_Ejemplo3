...
bool TextureShaderClass::Initialize(OpenGLClass* OpenGL, HWND hwnd)
{
	...
	result = InitializeShader((char*)"texture.vs", (char*)"texture.ps", OpenGL, hwnd);
	if(!result)
	{
		return false;
	}
...
bool TextureShaderClass::SetShaderParameters(OpenGLClass* OpenGL, float* worldMatrix, float* viewMatrix, float* projectionMatrix, int textureUnit, int vari)
{
	...
	location = OpenGL->glGetUniformLocation(m_shaderProgram, "vari2");
	if(location == -1)
	{
		return false;
	}
	OpenGL->glUniform1i(location, vari);
